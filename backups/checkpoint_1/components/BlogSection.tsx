...
export const BlogSection: React.FC<BlogSectionProps> = ({ blogs }) => {
  ...
  return (
    <section id="blog" className="py-24 bg-cream batik-pattern border-b border-gold/15 scroll-mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
          <div className="space-y-3 max-w-2xl">
            ...
            <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl font-bold text-royal-green tracking-wide">
              {t('blog.title')}
            </h2>
            <p className="text-sm sm:text-base text-royal-green/75 font-light leading-relaxed">
              {t('blog.subtitle')}
            </p>
          </div>
          ...
          <div className="relative w-full md:w-80">
            <Search className="w-4 h-4 text-royal-green/40 absolute left-4 top-1/2 transform -translate-y-1/2" />
            <input
              type="text"
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              placeholder="Search botanical wisdom..."
              className="w-full bg-white border border-gold/20 rounded-full pl-11 pr-5 py-3 text-sm focus:outline-none focus:border-gold shadow-sm"
            />
          </div>
        </div>
        ...
        {filteredBlogs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10" id="blog-grid">
            {filteredBlogs.map((post) => (
              <div 
                key={post.id}
                className="bg-white rounded-3xl border border-gold/15 overflow-hidden luxury-shadow hover:shadow-2xl transition-all duration-500 group flex flex-col h-full cursor-pointer"
              ...
              >
                ...
                <div className="relative aspect-16/9 overflow-hidden bg-royal-green">
                  ...
                  {/* Category overlay */}
                  <div className="absolute bottom-4 left-4 bg-royal-green/95 backdrop-blur-sm border border-gold/30 text-gold px-3.5 py-0.5 rounded-full text-xs font-mono font-medium uppercase tracking-wider">
                ...
                <div className="p-6 sm:p-8 flex flex-col flex-grow space-y-4">
                  {/* Date and Author */}
                  <div className="flex items-center space-x-4 text-xs font-mono text-royal-green/60">
                  ...
                  <div className="space-y-2">
                    <h3 className="font-serif text-xl sm:text-2xl font-bold text-royal-green group-hover:text-gold-dark transition-colors duration-300 line-clamp-2 leading-tight">
                      {translate(post.title)}
                    </h3>
                    <p className="text-xs sm:text-sm text-royal-green/75 font-light leading-relaxed line-clamp-3">
                      {translate(post.excerpt)}
                    </p>
                  </div>

                  {/* Read action CTA */}
                  <div className="pt-4 border-t border-gold/10 mt-auto flex items-center justify-between text-xs font-mono font-bold text-gold-dark group-hover:text-royal-green uppercase tracking-widest">
