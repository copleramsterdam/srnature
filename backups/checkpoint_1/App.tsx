...
import { Sparkles, Star, MapPin, RefreshCw, AlertCircle, ShoppingBag, Leaf, MessageCircle, CalendarRange, X } from 'lucide-react';
...
function MainAppContent() {
  ...
  useEffect(() => {
    if (seoSettings) {
      ...
      metaDesc.setAttribute('content', desc || "Premium Indonesian Herbal Jamu Website");
      ...
      metaKeys.setAttribute('content', keys || "Jamu, herbal, Indonesia");
    }
  ...
  const handleAddToOrder = (product: Product) => {
    if (selectedProducts.find(p => p.id === product.id)) {
      ...
      // Auto-focus scroll user to order checkout form
      const formEl = document.getElementById('order');
      if (formEl) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-cream flex flex-col items-center justify-center space-y-6" id="app-loading-screen">
        <div className="relative flex items-center justify-center">
          ...
          <div className="w-24 h-24 bg-royal-green rounded-full overflow-hidden shadow-2xl p-0.5 border-2 border-gold flex items-center justify-center animate-pulse">
            <img 
        ...
        <div className="text-center space-y-1">
          <span className="font-serif text-2xl font-bold text-royal-green tracking-wider uppercase block">
            JAVA HERBAL
          </span>
          <span className="text-xs font-mono text-gold-dark tracking-widest uppercase block animate-pulse">
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-cream flex flex-col items-center justify-center p-6 text-center space-y-4" id="app-error-screen">
        <div className="bg-red-100 p-4 rounded-full text-red-600 shadow-inner">
          <AlertCircle className="w-10 h-10" />
        </div>
        <div className="space-y-1.5 max-w-md">
          <h2 className="font-serif text-2xl font-bold text-royal-green">Botanical Server Desync</h2>
          <p className="text-sm text-royal-green/75 font-light leading-relaxed">{error}</p>
        </div>
        <button
          onClick={loadDatabase}
          className="bg-royal-green hover:bg-leaf-green text-cream px-6 py-2.5 rounded-full text-xs font-bold tracking-widest uppercase transition-colors"
        >
          Reconnect
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col" id="root-app-layout">
      ...
      <main className="flex-grow">
        ...
        {/* Brand Value Quality Pillars */}
        <Pillars />

        {/* Interactive Alchemical Herbal shop Section */}
        <section id="shop" className="py-24 bg-cream border-b border-gold/15 scroll-mt-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
            ...
            <div className="text-center max-w-3xl mx-auto space-y-4">
              ...
              <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl font-bold text-royal-green">
                {t('shop.title')}
              </h2>
              <p className="text-sm sm:text-base text-royal-green/75 font-light leading-relaxed max-w-xl mx-auto">
                {t('shop.subtitle')}
              </p>
            </div>
        ...
        {/* Educational History / About Us Section */}
        <AboutSection />

        {/* Philosophy of Balance / prevention section */}
        <PhilosophySection />

        {/* Curated Customer Review Testimonials Carousel */}
        <section id="reviews" className="py-20 bg-royal-green text-cream border-b border-gold/15 relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
            ...
            <div className="text-center max-w-2xl mx-auto space-y-3">
              ...
              <p className="text-sm text-cream/70 font-light font-sans">
                {t('reviews.subtitle')}
              </p>
            </div>
        ...
        {/* Live Botanical Chronicles / Blog Section */}
        <BlogSection blogs={blogs} />

        {/* Booking Form desk / Checkout Shopping basket */}
        <OrderForm
        ...
        {/* Secure Management Sanctum Panel for Products, Blogs, Orders, and AI tools */}
        {activeSection === 'admin' && (
          <AdminPanel
            products={products}
            blogs={blogs}
        ...
        <AnimatePresence>
          {isWhatsAppMenuOpen && (
            <motion.div
              ...
              className="bg-white border-2 border-gold rounded-2xl p-4 shadow-2xl mb-4 w-72 flex flex-col space-y-3 relative overflow-hidden"
              ...
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  <h4 className="font-serif text-sm font-bold text-royal-green leading-tight">
                    {language === 'nl' ? 'Jamu Specialist' : 'Jamu Specialist'}
                  </h4>
