...
import { Order, Product, BlogPost, DatabaseSchema } from './src/types';
...
app.post('/api/orders', (req, res) => {
  ...
  if (!customerName || !phone || !productIds || !productIds.length) {
    return res.status(400).json({ error: 'Missing required customer details or products.' });
  }
  ...
  const newOrder: Order = {
    ...
    productIds,
    productNames,
    quantities: quantities || productIds.map(() => 1),
    prices,
    totalPrice,
    notes: notes || '',
    status: 'pending',
  ...
  let waText = `🌿 *Djamoe Heritage - New Herbal Order* 🌿\n\n`;
  ...
  productIds.forEach((id: string, idx: number) => {
    const prodName = productNames[idx];
  ...
  waText += `\nThank you for choosing Javanese Heritage Wellness. Please confirm my order!`;
...
The article is for a luxurious Indonesian Herbal Jamu brand located in Amsterdam, the Netherlands called "Djamoe Heritage".
Focus on Javanese wisdom, natural remedies, organic lifestyle, and ancient history.
